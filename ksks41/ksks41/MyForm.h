...
#pragma region Windows Form Designer generated code
        ...
        void showing(int index, int x, int y) {
            for (const Sprite& sprite : sprites) {
                if (sprite.index == index) {
                    ...
                    spriteGraphics->DrawImage(g, IntParametrs[0], IntParametrs[1], IntParametrs[2], IntParametrs[3], sprite.data);
        ...
        void parsi(String^ strings_mes)
        {
            ...
            if (ValidCommand == false)
            {
                ...
                if (ricyem == 20)
                {
                    loading(IntParametrs[0], IntParametrs[1], IntParametrs[2], DataIm);
                }
                if (ricyem == 21)
                {
                    showing(IntParametrs[0], IntParametrs[1], IntParametrs[2]);
                }
